      ...
      exception
        when unique_violation then
          ...
          if v_folder_id is not null then
            ...
            set
              emoji = coalesce(v_folder_emoji, emoji),
              client_id = coalesce(client_id, v_local_id),
              client_version = greatest(coalesce(client_version, 1), v_version),
              client_updated_at = greatest(coalesce(client_updated_at, to_timestamp(0)), v_updated_at)
            where id = v_folder_id
              and user_id = p_user_id;
          end if;
      end;
