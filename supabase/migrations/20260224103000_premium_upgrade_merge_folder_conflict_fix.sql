...
begin
  ...
  loop
    ...
    if v_folder_id is null then
      ...
      on conflict on constraint folders_user_name_unique
      do update set
        emoji = excluded.emoji,
        client_id = coalesce(public.folders.client_id, excluded.client_id),
        client_version = greatest(coalesce(public.folders.client_version, 1), excluded.client_version),
        client_updated_at = greatest(coalesce(public.folders.client_updated_at, to_timestamp(0)), excluded.client_updated_at)
  ...
  loop
    ...
    loop
      ...
      if v_folder_id is null then
        ...
        on conflict on constraint folders_user_name_unique
        do update set
          emoji = coalesce(public.folders.emoji, excluded.emoji)
        returning id into v_folder_id;
      end if;
